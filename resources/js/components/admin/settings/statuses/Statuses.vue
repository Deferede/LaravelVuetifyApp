              ...
              <template v-slot:top>
                ...
                <v-dialog v-model="dialogNew" max-width="35%">
                  <card-create :title="item.name" @close="closeDialogs" @confirm="confirmCreate">
                    ...
                    <v-text-field label="Priority" type="number" min="0" v-model="item.priority"
                                  required></v-text-field>
                    <v-select label="Type" v-model="item.type" :items="itemTypes" item-value="id"
                              item-text="name"></v-select>
                    <v-select label="Category" v-model="item.category" :items="itemCategories" item-value="id"
                              item-text="name"></v-select>
                  </card-create>
                </v-dialog>
                <!--END-->
                ...
                <v-dialog v-model="dialogEdit" max-width="35%">
                  <card-edit :title="item.name" @close="closeDialogs" @confirm="confirmUpdate">
                    <v-text-field label="Name" type="text" v-model="item.name" required></v-text-field>
                    <v-text-field label="Priority" type="number" aria-valuemin="0" v-model="item.priority"
                                  required></v-text-field>
                    <v-select label="Role" v-model="item.type" :items="itemTypes" item-value="id"
                              item-text="name"></v-select>
                    <v-select label="Role" v-model="item.category" :items="itemCategories" item-value="id"
                              item-text="name"></v-select>
                  </card-edit>
                </v-dialog>
                <!--END-->
  ...
  export default {
    ...
    computed: {
      ...mapGetters({
        ...
        item: 'statuses/getItem',
        items: 'statuses/getItems',
        meta: 'shared/meta',
        itemTypes: 'statusTypes/item',
        itemCategories: 'statusCategories/getItems'
      }),
    },
    watch: {
      search() {
    ...
    methods: {
      ...mapActions({
        createItem: 'statuses/createItem',
        updateItem: 'statuses/updateItem',
        destroyItem: 'statuses/deleteItem',
        resetItem: 'statuses/loadBlankItem',
        loadItem: 'statuses/loadItem',
        loadItems: 'statuses/loadItems',
        loadItemTypes: 'statusTypes/loadItems',
        loadItemCategories: 'statusCategories/loadItems'
      }),
      newItem() {
        this.resetItem().then(() => {
          this.dialogNew = true
